    ...
    async initModel() {
      ...
      if (!this.unsupported) {
        try {
          ...
          // this.faceModel = new faceapi.SsdMobilenetv1()
          // await this.faceModel.load('/demo-tensorflowjs/models/face-mobilenet-v1')
          await faceapi.loadSsdMobilenetv1Model('/demo-tensorflowjs/models/face-mobilenet-v1')
          this.faceModel = true
          this.emotionModel = await this.$tf.loadLayersModel(
            '/demo-tensorflowjs/models/emotion/model.json'
          )
          this.startCam()
        } catch (error) {
          console.error(error)
    ...
    async predictWebcam() {
      try {
        ...
        if (this.faceModel && this.emotionModel) {
          ...
          const predictions = await faceapi.detectAllFaces(this.videoCanvas, new faceapi.SsdMobilenetv1Options())
          const predictionsResize = faceapi.resizeResults(predictions, { width: this.mWidth, height: this.mHeight })
          for (const item of predictionsResize) {
            const score = item.score
             if (score <= 0.6) continue
            const start = item.box.topLeft

            this.videoContext.beginPath()
            this.videoContext.strokeStyle = '#fefefe'
            ...
            if (maxIdx >= 0 && predictedValue[0][maxIdx] > 0.6) {
              ...
              // console.log(label, predictedValue[0][maxIdx])
